...
export type ValidationResult = 
  | { success: true; data: CharacterDataValidation }
  | { success: false; errors: string[] };

/**
 * Validates a JSON string against the character schema.
 * Returns the parsed data on success, or a list of formatted error messages on failure.
 */
export function validateCharacterJson(jsonText: string): ValidationResult {
  try {
    ...
    return { success: false, errors: [`Failed to parse JSON string: ${message}`] };
  }
}
